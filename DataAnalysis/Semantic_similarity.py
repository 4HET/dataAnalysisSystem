...
def get_value(text):
    url = "https://aip.baidubce.com/rpc/2.0/nlp/v2/word_emb_sim?access_token={}&charset=UTF-8".format(access_token)
    data = {
        "word_1": text[0],
        "word_2": text[1]
    }
...
def semantic_similarity(file_id, file_name):
    get_token()
    ...
    for i in fi:
        ...
        with open("DataAnalysis/per_data/" + file_n + ".csv", "a", encoding="utf-8", newline="") as f:
            f = csv.writer(f)
            f.writerow(ls)
    sql = 'insert into file_info(file_name, file_id, file_user) values("{}", "{}", "{}");'.format("词义相似度结果_" + file_name, file_n, "123")
    mysql.mysql(sql)